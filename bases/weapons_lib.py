...
def MakeWeapon(concourse,timeofdayignored='_day', dealername="bases/repair_upgrade/shipdealer",upgradename='bases/repair_upgrade/shipupgrade',use_ship_320_240_upgrade=True):
	...
	mastPL = VS.GetMasterPartList()
	centPrice = mastPL.GetCargo('centurion.begin').GetPrice()
	orPrice = mastPL.GetCargo('orion.begin').GetPrice()
	galPrice = mastPL.GetCargo('galaxy.begin').GetPrice()
	
	#playerShipName=VS.getPlayer().getName()
	#tradeInValue=CargoValue(VS.getPlayer())+ShipValue(playerShipName,True)

	# add clickable areas to the ship dealer
	Base.Python (room_ship_dealer, 'my_comp_id', -0.9925, -0.263333, 0.8875, 0.56, 'Purchase Centurion, cost: ' + str(int(centPrice)), '#\nimport weapons_lib\nweapons_lib.ShipPurchase(\'centurion\')\n',True)
	Base.Python (room_ship_dealer, 'my_comp_id', -0.6825, -0.973333, 0.77, 0.516667, 'Purchase Orion, cost: ' + str(int(orPrice)), '#\nimport weapons_lib\nweapons_lib.ShipPurchase(\'orion\')\n',True)
	Base.Python (room_ship_dealer, 'my_comp_id', 0.2925, -0.96, 0.68, 0.733333, 'Purchase Galaxy, cost: ' + str(int(galPrice)), '#\nimport weapons_lib\nweapons_lib.ShipPurchase(\'galaxy\')\n',True)
	...
	def reset(self):
		...
		if (move_mounts):
			...
			if ship_name == "tarsus":
				...
				self.move_mount_selector("btn_mount_1", GUI.GUIRect(230, 110, w,  h))   #PODXX added this mount
				self.move_mount_selector("btn_mount_2", GUI.GUIRect(183, 101, w,  h))
				self.move_mount_selector("btn_mount_3", GUI.GUIRect(277, 101, w,  h))
				self.move_mount_selector("btn_mount_c", GUI.GUIRect(90, 168, wc, hc))	# cancel button
			elif ship_name == "galaxy":
				...
				self.move_mount_selector("btn_mount_3", GUI.GUIRect(125, 83, w,  h))    #PODXX switched places
				self.move_mount_selector("btn_mount_1", GUI.GUIRect(140, 83, w,  h))
				self.move_mount_selector("btn_mount_2", GUI.GUIRect(278, 80, w,  h))
...
def CargoValue(un):
	...
	for i in range(numcarg):
		...
		if category[:14] == 'upgrades/Radar':                #PODXXX fixed incorrect trade-in price for radars
		    print "PODXX radar " + category + ", price: " + str(c.GetPrice())
		    tot+=c.GetPrice()*c.GetQuantity()*.75
		elif c.GetCategory().find("upgrades")==0:
		    print "PODXX " + category + ", price: " + str(c.GetPrice())
		    tot+=c.GetPrice()*c.GetQuantity()*.75            #PODXX was .5 and worked incorrectly; now it's closer but still won't equal to a manual-sell amount...
	return tot;
