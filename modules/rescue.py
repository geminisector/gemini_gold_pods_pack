...
class rescue (Director.Mission):
    ...
    def __init__ (self, creds,numsysaway, fac,numenemy,enfac,enfg='', jumps=(),var_when_done=''):
        ...
        self.numejectors=0
        self.obj=0
        self.enemyChasePilot = vsrandom.random() > .3
...
#            self.eject = launch.launch_wave_around_unit("Rescue Pilot",self.faction,"eject","printhello.py",1,mymin,mymax,self.adjsys.SignificantUnit())
            self.eject = launch.launch_wave_around_unit("Rescue Pilot",self.faction,"eject","printhello.py",1,mymin,mymax,self.you)
...
#            self.enemy=L.launch(self.adjsys.SignificantUnit())
            self.enemy=L.launch(self.eject)
            if self.enemyChasePilot:      #PODXX giving a chance for eject to avoid attack
            ...
            if (self.eject):
                ...
                if (self.numejectors<self.you.GetCargo("Pilot").GetQuantity()):
                    ...
                    if (not self.you.addCargo(carg)):
                        ...
                        VS.addObjective("Take pilot to " + unit.getUnitFullName(self.adjsys.SignificantUnit(),True))   #PODXX
