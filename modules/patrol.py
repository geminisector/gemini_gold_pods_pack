...
class patrol (Director.Mission):
    def __init__ (self,numsystemsaway, num_significants_to_patrol, distance_from_base, creds, jumps=(), donevar='', patrolPointsNames = []):
        Director.Mission.__init__(self)
        self.donevar=donevar
        self.jnum=0
        self.cred=creds
        self.patrolpoints = []
        self.patrolPointsNames = patrolPointsNames
        self.objectives = []
    ...
    def GeneratePatrolList (self):
        ...
        for ppn in self.patrolPointsNames:
            self.patrolpoints.append(universe.findUnit(ppn))
        if not len(self.patrolpoints):
            self.patrolpoints=universe.significantUnits()
        
        #PODXX removing asteroid fields, they always "shadow" normal nav points
        tpp = []
        for isig in range(len(self.patrolpoints)):
            signame = self.patrolpoints[isig].getName()
#            print "PODXX signame[" + signame + "] isPlanet[" + str(self.patrolpoints[isig].isPlanet()) + "]"
            if (signame != "") and (signame.find("Asteroid") < 0):
                tpp.append(self.patrolpoints[isig])
        self.patrolpoints = tpp
        ...
        for sig in self.patrolpoints:
            ...
            if (fac!="neutral"):
                ...
                if (sig.isPlanet()):
                    nam =unit.getUnitFullName(sig)
                    if (sig.isJumppoint()):
                        obj=VS.addObjective ("Scan: %s" % nam)            #PODXX shorter task fits VDU better
                    else:
                        obj=VS.addObjective ("Scan: %s" % nam)
                else:
                    obj=VS.addObjective ("Scan: %s" % nam)
