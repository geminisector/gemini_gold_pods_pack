...
def NotEnoughCargoRoom(plr,reqspace):
        #PODXX added reqspace support
	player=VS.getPlayerX(plr)

	cargo_obj = VS.Cargo("SpaceSalvage","SpaceSalvage", 185.5, reqspace , 0.01, 1.0)
	ret=player.addCargo(cargo_obj)
	print "Enough cargo room for ["+str(ret)+"], need [" + str(reqspace) + "]"
	if ret:
...
def LoadLastMission(which=None):
	...
	ret = True
        #PODXX added numcargos support
	if len(last_briefing_vars) and which in last_briefing_vars[0] and 'MISSION_TYPE' in last_briefing_vars[0][which] and last_briefing_vars[0][which]['MISSION_TYPE']=='CARGO' and NotEnoughCargoRoom(plr,int(last_briefing_vars[0][which]['NUMCARGOS'])):
		ret= False
...
def RemoveActiveMissionEntry(entry_id):
	plr = getMissionPlayer()
	players[plr].active_missions = filter(lambda x:x.get('ENTRY_ID',None)!=entry_id, players[plr].active_missions)
...
def DeleteOldMissions(plr,guildName):
	last_briefing = players[plr].last_briefing
#	print "PODXX last_briefing before: " + str(last_briefing)
	tmp = []
	for which in last_briefing[0]:
		if str(which).startswith(guildName):
#			print "PODXX nextMis[" + str(which) + "] guildName[" + guildName + "]"
			tmp.append(which)

	for which in tmp:
		RemoveLastMission(which)
#	print "PODXX last_briefing after: " + str(last_briefing)
...
def CreateGuildMissions(guildname,nummissions,accepttypes,prefix="#G#",acceptmsg=''):
	...
	import trading
        (baseName, faction) = trading.getUniqueBaseNameAndFaction(universe.getDockedBase())

	guildID = baseName + "_" + guildname
	global refreshTimestamps
	
	print "PODXXX CreateGuildMissions ID[" + guildID + "] time: " + str(VS.GetGameTime())
	try:
            prevRefreshTime = refreshTimestamps[guildID]
            if VS.GetGameTime() - prevRefreshTime < 25:
                return CountMissions(0,guildname)
        except:
            pass

        print "PODXXX will refresh missions"
        refreshTimestamps[guildID] = VS.GetGameTime()
	DeleteOldMissions(plr,guildname)  #delete missions left from previous base or prior to regeneration
	...
	for indx in range(Director.getSaveStringLength(plr, "mission_scripts")):
		...
		if (script.find(prefix)!=-1):
			...
			if (accepttypes==None):
				...
				freq = accepttypes['*']
			#print "PODXX guild [" + guildname + "] missiontype [" + missiontype + "] freq [" + str(freq) + "]" 
			if freq:
