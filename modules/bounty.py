...
class bounty (Director.Mission):
	...
	self.greetTime = 10000000
        ...
        self.enrouteObjective = enrouteObjective
        self.adjsys = 0
        if enrouteObjective == '':
            self.adjsys=go_to_adjacent_systems (self.you,vsrandom.randrange(minnumsystemsaway,maxnumsystemsaway+1),jumps)
        else:
            #print "PODXX bounty route: " + str(jumps)
            self.adjsys=go_to_adjacent_systems (self.you,vsrandom.randrange(minnumsystemsaway,maxnumsystemsaway+1),jumps,'',False)
            self.obj = VS.addObjective(self.enrouteObjective)
        ...
        if (self.arrived==2):
            ...
            if (significant.isNull ()):
                ...
                if (self.you.getSignificantDistance(significant)<self.adjsys.distfrombase):
                    ...
                    L.num=1
                    L.minradius=1200.0
                    L.maxradius = 100.0 * vsrandom.randrange(13,25)
                    self.enemy=L.launch(self.you)
                    isLockOnYou = False
                    if vsrandom.random > .5:      #PODXX it's more realistic for the prey to attack you only sometimes
                      self.enemy.SetTarget(self.you)
                      isLockOnYou = True
                    self.enemy.setFgDirective('A.')
                    self.you.SetTarget(self.enemy)
                    
                    if self.escortShipType <> '':
                      L = launch.Launch()
                      L.fg="Shadow"
                      L.dynfg=self.dynfg
                      L.type = self.escortShipType
                      L.faction = self.faction
                      L.ai = "default"
                      L.num=1
                      L.minradius = 25
                      L.maxradius = 50
                      self.escort=L.launch(self.enemy)
                      if isLockOnYou:
                        self.escort.SetTarget(self.you)
                      self.escort.setFgDirective('A.')
                    
                    self.greetTime=VS.GetGameTime()+6
                    unitName = unit.getUnitFullName(self.enemy,False)
                    colonIn = unitName.find(":")
                    if (colonIn!=-1):
                        unitName = unitName[0:colonIn].capitalize()
                    commaIn = unitName.find(".")
                    if (commaIn!=-1):
                        unitName = unitName[0:commaIn]
                    
                    if self.faction <> "unknown":
                        #self.obj=VS.addObjective("Destroy " + self.faction.capitalize() + " " + unitName)
                        VS.setObjective(self.obj, "Destroy " + self.faction.capitalize() + " " + unitName)
                    else:
                        VS.setObjective(self.obj, "Destroy " + unitName)
                    if (self.enemy):
                        if (self.runaway):
            ...
            if (self.adjsys.Execute()):
                ...
                if (self.newship=="" and self.dynfg==''):
                    self.newship=faction_ships.getRandomFighter(self.faction)
                self.adjsys=go_somewhere_significant(self.you,self.dockable_unit,10000.0,0,'','',self.displayLocation,0,self.predefinedSignificantName)
                if not self.displayLocation and self.enrouteObjective=='':
                    self.obj = VS.addObjective("Hunt down "+self.faction.capitalize()+" vessel");
